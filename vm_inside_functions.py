...
def get_PID(exe_name):
    ...
    # runs the upper line through cmd, prints tasklist to a text file
    table_create = subprocess.check_output(
        table_cmd, shell=True, universal_newlines=True)
...
def get_IP(pid_list):
    ...
    # prints netstat into a text file through cmd
    table_create = subprocess.check_output(
        netstat_cmd, shell=True, universal_newlines=True)
    file = open('netstat.txt', 'r')
    ip_list = []  # the list which will be returned
    for pid in pid_list:
        # takes only Foreign IPs from netstat.txt and checks they are unique
        ip_cmd = 'awk "$5 == '+pid + \
            ' { print $0 }" netstat.txt|awk "{ print $3 }"|awk -F: " { print $1 }"|sort /uniq'
        list_create = subprocess.check_output(
            ip_cmd, shell=True, universal_newlines=True)
...
def which_ports(ip_list):
    ...
    for IP in ip_list:
        in_port = subprocess.check_output(
            'more netstat.txt|findstr " '+IP+'"|awk "{ print $2 }"|awk -F: "{ print $2 }"', shell=True, universal_newlines=True)
        in_port = in_port.split()
        out_port = subprocess.check_output(
            'more netstat.txt|findstr " '+IP+'"|awk "{ print $3 }"|awk -F: "{ print $2 }"', shell=True, universal_newlines=True)
        out_port = out_port.split()
        # write all the in and out ports with their use into a file
        text_file = open("ports.txt", "a+")
        text_file.write("In ports:")
        text_file.write('\n')
        for port in in_port:
            try:  # not all the ports are known (1-1024 are assigned)
                socket.getservbyport(int(port))
                text_file.write(str(port)+" is " +
                                socket.getservbyport(int(port)))
                text_file.write('\n')  # new line
...
def where_from_IP(ip_list):
    ...
    for IP in ip_list:
        try:  # some IPs have an unknown location
            ...
            text_file.write("IP: " + IP + "    Country:   "+response.country +
                            "    Region:   "+response.region+"    City:   "+response.city)
            text_file.write('\n')
        except KeyError:
            pass
    text_file.close()  # close the file
...
def send_mail(mail):
    ...
    points = 0
    # if file is empty, there are no blacklisted IP so no points
    if os.stat('ip_results.txt').st_size != 0:
        points += 2
        ...
        if line == 'Out Ports:':
            while line != 'In Ports:':
                if line.find('unknown') != -1:
                    ...
                    break
    fline = 'your .exe got ' + \
        str(points) + ' out of 3. 0 means clean and 3 means most dangerous.'
...
def main():
    try:
        ...
        print('There was a problem sending the mail in your wifi. Please contact the developer')
